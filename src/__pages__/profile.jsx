...
import { update } from '../__redux__/thunk/auth.thunk';
...
const Profile = () => {
    ...
    if (!user) return <MessageDisplay message={generic_msg.resource_not_found('User')} />;
    ...
    const handleUpdate = async (e) => {
        ...
        if (!isProfileChanged) {
            toast.info(generic_msg.no_change_found_in_profile);
            setIsEditing(false);
            return;
        }
        try {
            ...
            if (update.fulfilled.match(resultAction)) {
                ...
                toast.error(resultAction.error.message || generic_msg.profile_update_fails);
            }
        } catch (error) {
